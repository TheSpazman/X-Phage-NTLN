...
tbox_x = camera_get_view_x( view_camera[0] ) +76;
tbox_y = camera_get_view_y( view_camera[0] ) +340;
...
if tbox_setup == false 
	{
		...
		if instance_exists(obj_player) { obj_player.playerCanMove = 0; }
		...
		for (var p = 0; p < tbox_page_number; p++)
			{
				...
				//Get X-Position of Textbox
					// Character on Left / Right / Center (Simplified from tutorial, since the portraits would always be visible / present, no offsetting. 
						tbox_text_x_offset[p] = 28; 
						left_vdu_x_offset[p] = -76; 
						left_vdu_y_offset[p] = -16;
						right_vdu_x_offset[p] = 488;
						right_vdu_y_offset[p] = -16;
...
if accept_key
	...
	//If not done typing... 
		...
		if instance_exists(obj_player) { obj_player.playerCanMove = 1; }
		
	}


//----- Draw the textbox. 
var _tbox_x = tbox_x;						// Textbox X-Position
...
if  right_vdu_sprite[tbox_page] != noone 
	{
		...
		draw_sprite_ext(sprite_index, image_index,_rightvdu_x +14, _rightvdu_y +4,1,1,0,c_white,1); // Drawing Right VDU Contents
		draw_sprite_ext(tbox_rvdu_border_flicker, tbox_rvdu_border_flicker_speed,_rightvdu_x +12, _rightvdu_y +4,1,1,0,c_white,0.2); // Drawing Right VDU Flicker
